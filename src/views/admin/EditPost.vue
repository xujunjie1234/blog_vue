...
import {addPost, uploadImage, editPost} from "network/admin"
import {getCategories, getPost} from "network/home"
	...
	methods: {
		...
		_get_post(post_id){
			getPost(post_id).then(res => {
				...
				this.category_id = res.post.category_id
			})
		},
		goBack(){
			this.$router.go(-1)
